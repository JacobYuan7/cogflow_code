...
def create_one_query(problem, examples, shot_num, shot_type, use_caption, use_ocr, mode):
    ### [1] Demo prompt
    if shot_num == 0:
        demo_prompt = ""
    else:
    ...
    if shot_type == "solution":
        if question_type == "multi_choice":
            ...
            if answer_type == "integer":
                ...
                hint_text = f"Hint: Please answer the question and provide the final text, e.g., 'apple', 'banana', 'cherry', at the end."
    ...
    # question
    if mode == 'Text_Centric':
        que = problem['DI'] + problem['IP'] + problem['EC1'] + problem['EC2'] + problem['only_question']
    elif mode == 'Text_Limited':
        que = problem['IP'] + problem['EC1'] + problem['EC2'] + problem['only_question']
    elif mode == 'Vision_Dense':
        que = problem['EC1'] + problem['EC2'] + problem['only_question']
    elif mode == 'Vision_Centric':
        que = problem['IP'] + problem['only_question']
    elif mode == 'Vision_Primary':
        que = problem['only_question']
    elif mode == 'Text_Plus':
        que = problem['fully_question'] + problem['IP']    
    else:
        que = problem["question"]
    que = que.replace("None", "") # 去掉None    
    question_text = f"Question: {que}"
    # if unit:
    ...
    ### [3] Final query
    query = demo_prompt + "\n\n" + test_query + '\n\n' + 'Give the answer to the question as directly as possible'
    query = query.strip()
    return query

def create_one_query_cot(problem, step_text, mode):
    ...
    # solution step
    solution_text = step_text 

    # choices
    if isinstance(choices, list) and choices[0] != 'None':
        # choices: (A) 1.2 (B) 1.3 (C) 1.4 (D) 1.5
        texts = ["Choices:"]
        for i, choice in enumerate(choices):
            texts.append(f"({chr(ord('A')+i)}) {choice}")
    ...
    prompt = "Answer: "
    elements = [question_text, solution_text, choices_text, hint_text, prompt]
    test_query = "\n".join([e for e in elements if e != ""])
